...
class TestFrontendAccessibility:
    ...
    def test_frontend_loading(self, service_checker, config):
        ...
        assert info["status_code"] == 200, f"Frontend returned status {info['status_code']}"
...
class TestFrontendPerformance:
    ...
    def test_frontend_response_size(self, service_checker, config):
        ...
        if content_length:
            ...
            assert size_in_kb < 1000, f"Frontend response size {size_in_kb:.1f}KB is too large (expected < 1000KB)"
...
class TestFrontendContent:
    ...
    def test_frontend_content_loading(self, service_checker, config):
        ...
        for check in content_checks:
            assert check.lower() in html_content.lower(), f"Frontend missing content: {check}"
...
class TestFrontendResponsiveness:
    ...
    def test_mobile_viewport(self, service_checker, config):
        ...
        html_content = info["data"]
        assert "viewport" in html_content, "Frontend missing viewport meta tag"
        assert "width=device-width" in html_content, "Frontend missing responsive viewport"
...
class TestFrontendIntegration:
    ...
    def test_backend_api_connectivity_from_frontend(self, service_checker, config):
        ...
        assert service_checker.check_service_health(backend_health_url), "Backend not accessible from frontend perspective"
    ...
    def test_api_documentation_links(self, service_checker, config):
        ...
        assert service_checker.check_service_health(docs_url), "API documentation not accessible"
    ...
    def test_health_check_links(self, service_checker, config):
        ...
        assert service_checker.check_service_health(health_url), "Health check not accessible"
