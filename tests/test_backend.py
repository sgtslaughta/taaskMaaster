...
class TestBackendAPI:
    ...
    def test_root_endpoint(self, service_checker, config):
        ...
        assert info["status_code"] == 200, f"Root endpoint returned status {info['status_code']}"
    ...
    def test_health_endpoint(self, service_checker, config):
        ...
        assert info["status_code"] == 200, f"Health endpoint returned status {info['status_code']}"
    ...
    def test_metrics_endpoint(self, service_checker, config):
        ...
        assert info["status_code"] == 200, f"Metrics endpoint returned status {info['status_code']}"
        
        # Check that it returns Prometheus metrics format
        content = info["data"]
        assert "http_requests_total" in content, "Metrics missing http_requests_total"
        assert "http_request_duration_seconds" in content, "Metrics missing http_request_duration_seconds"
    ...
    def test_api_status_endpoint(self, service_checker, config):
        ...
        assert info["status_code"] == 200, f"API status returned status {info['status_code']}"
        
        data = info["data"]
        assert "api_version" in data, "API status missing api_version field"
        assert data["api_version"] == "v1", f"API version is {data['api_version']}, expected 'v1'"
        ...
        expected_features = ["task_management", "user_management", "gamification", "file_storage"]
    ...
    def test_documentation_endpoints(self, service_checker, config):
        ...
        assert info["status_code"] == 200, f"API docs returned status {info['status_code']}"
        assert "text/html" in info["headers"].get("content-type", ""), "API docs not returning HTML"
        ...
        assert info["status_code"] == 200, f"ReDoc returned status {info['status_code']}"
        assert "text/html" in info["headers"].get("content-type", ""), "ReDoc not returning HTML"
...
class TestBackendSecurity:
    ...
    def test_cors_configuration(self, service_checker, config):
        ...
        try:
            ...
            assert "http://localhost:3000" in allowed_origins or allowed_origins == "*", "Frontend origin not allowed"
...
class TestBackendPerformance:
    ...
    def test_response_time(self, service_checker, config):
        ...
        for endpoint in endpoints:
            ...
            assert response_time < 2.0, f"Response time for {endpoint}: {response_time:.2f}s (expected < 2.0s)"
    ...
    def test_concurrent_requests(self, service_checker, config):
        ...
        for response_time in results:
            assert response_time < 3.0, f"Concurrent request response time: {response_time:.2f}s (expected < 3.0s)"
