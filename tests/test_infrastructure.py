...
class TestDockerEnvironment:
    ...
    def test_docker_compose_available(self, service_checker):
        ...
        assert service_checker.check_docker_compose_available(), "Docker Compose is not available"
    
    def test_docker_compose_config_valid(self, service_checker):
        """Test that Docker Compose configuration is valid."""
        assert service_checker.check_docker_compose_config(), "Docker Compose configuration is invalid"
...
class TestServiceStatus:
    ...
    def test_backend_container_running(self, service_checker):
        ...
        assert service_checker.check_container_running("taaskmaaster-backend"), "Backend container is not running"
    
    def test_frontend_container_running(self, service_checker):
        """Test that frontend container is running."""
        assert service_checker.check_container_running("taaskmaaster-frontend"), "Frontend container is not running"
    
    def test_redis_container_running(self, service_checker):
        """Test that Redis container is running."""
        assert service_checker.check_container_running("taaskmaaster-redis"), "Redis container is not running"
    
    def test_minio_container_running(self, service_checker):
        """Test that MinIO container is running."""
        assert service_checker.check_container_running("taaskmaaster-minio"), "MinIO container is not running"
...
class TestServiceHealth:
    ...
    def test_backend_health(self, service_checker, config):
        ...
        assert service_checker.check_service_health(health_url), "Backend health check failed"
    
    @pytest.mark.slow
    def test_frontend_health(self, service_checker, config):
        """Test frontend health."""
        assert service_checker.check_service_health(config.frontend_url), "Frontend health check failed"
    ...
    def test_backend_health_response(self, service_checker, config):
        ...
        assert info["status_code"] == 200, f"Backend health returned status {info['status_code']}"
        ...
        for service in expected_services:
            ...
            assert services[service] in ["healthy", "unhealthy"], f"Invalid {service} status: {services[service]}"
...
class TestPortAccessibility:
    ...
    def test_backend_port_accessible(self, service_checker, config):
        ...
        assert service_checker.check_service_health(config.backend_url), "Backend port is not accessible"
    
    def test_frontend_port_accessible(self, service_checker, config):
        """Test that frontend port is accessible."""
        assert service_checker.check_service_health(config.frontend_url), "Frontend port is not accessible"
...
class TestAPIEndpoints:
    ...
    def test_api_documentation_accessible(self, service_checker, config):
        ...
        assert service_checker.check_service_health(docs_url), "API documentation is not accessible"
    ...
    def test_openapi_spec_accessible(self, service_checker, config):
        ...
        assert info["status_code"] == 200, f"OpenAPI spec returned status {info['status_code']}"
    ...
    def test_api_root_endpoint(self, service_checker, config):
        ...
        assert info["status_code"] == 200, f"API root returned status {info['status_code']}"
...
class TestPerformance:
    ...
    def test_backend_response_time(self, service_checker, config):
        ...
        assert info is not None, "Backend not responding"
        assert response_time < 2.0, f"Backend response time {response_time:.2f}s is too slow (expected < 2.0s)"
    ...
    def test_frontend_response_time(self, service_checker, config):
        ...
        assert response_time < 5.0, f"Frontend response time {response_time:.2f}s is too slow (expected < 5.0s)"
...
class TestErrorHandling:
    ...
    def test_404_handling(self, service_checker, config):
        ...
        try:
            response = requests.get(f"{config.backend_url}/non-existent-endpoint", timeout=10)
