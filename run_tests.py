...
# Import our logger
from tests.logger import setup_logging, get_test_logger
...
class TestRunner:
    ...
    def __init__(self):
        ...
        self.logger.logger.info("TestRunner initialized", extra={
    ...
    def check_docker_build(self) -> bool:
        ...
        if result["success"] and result["stdout"].strip():
            ...
            build_result = self.run_command(compose_cmd + ["build", "backend"], timeout=300)
            if not build_result["success"]:
                print("❌ Backend build failed:")
                print(build_result["stderr"])
                self.logger.logger.error("Backend build failed", extra={
        ...
        if result["success"] and result["stdout"].strip():
            ...
            build_result = self.run_command(compose_cmd + ["build", "frontend"], timeout=300)
            if not build_result["success"]:
                print("❌ Frontend build failed:")
                print(build_result["stderr"])
                self.logger.logger.error("Frontend build failed", extra={
                    "stderr": build_result["stderr"]
                })
                return False
    ...
    def _ensure_services_running(self) -> bool:
        ...
        if not result["success"]:
            self.logger.logger.error("Failed to start services", extra={
                "stderr": result["stderr"]
            })
            return False
    ...
    def run_infrastructure_tests(self, verbose: bool = False) -> bool:
        ...
        if not self._ensure_services_running():
            self.logger.logger.error("Failed to start services for infrastructure tests")
            return False
        
        cmd = [
            sys.executable, "-m", "pytest", "tests/test_infrastructure.py",
            "--tb=short",
            "--strict-markers",
            "--durations=10",
        ]
    ...
    def run_backend_tests(self, verbose: bool = False) -> bool:
        ...
        cmd = [
            sys.executable, "-m", "pytest", "tests/test_backend.py",
            "--tb=short",
            "--strict-markers",
            "--durations=10",
        ]
    ...
    def run_frontend_tests(self, verbose: bool = False) -> bool:
        ...
        cmd = [
            sys.executable, "-m", "pytest", "tests/test_frontend.py",
            "--tb=short",
            "--strict-markers",
            "--durations=10",
        ]
    ...
    def _display_test_results(self, result: Dict[str, Any], test_name: str):
        ...
        for line in lines:
            if "FAILED" in line or "ERROR" in line or "passed" in line or "failed" in line:
...
def main():
    ...
    parser.add_argument("test_type", nargs="?", default="all", 
                       choices=["all", "quick", "infrastructure", "backend", "frontend", "integration", "coverage"],
                       help="Type of tests to run")
    parser.add_argument("-v", "--verbose", action="store_true", help="Verbose output")
    parser.add_argument("-s", "--start", action="store_true", help="Start services before testing")
    parser.add_argument("-S", "--stop", action="store_true", help="Stop services after testing")
    ...
    # Log test run start
    runner.logger.logger.info("Test run started", extra={
    ...
    try:
        ...
        runner.logger.logger.info("Test run completed", extra={
            "success": success,
